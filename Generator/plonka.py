...
def quad(a_b, c_d):
    return np.vstack((np.hstack(a_b), np.hstack(c_d)))
...
def add_m(n, b, modified):
    ...
    if not modified:
        if b == 0:
            ...
            m = sqrt1_2 * quad((I(n1-1), I(n1-1)), (I(n1-1), -I(n1-1)))
            op1 = diag(diag(1, m), -1)
        ...
        if b == 1:
            ...
            op1 = diag(I(n1), J(n1))
            op2m = quint(I(n1-1), J(n1-1), J(n1-1), -I(n1-1), sqrt2)
...
def twiddle_m(n, b, modified):
    ...
    if not modified:
        if b == 0:
            return sqrt1_2 * quad((I(n1), J(n1)), (I(n1), -J(n1)))
        elif b == 1:
            m00 = diag(c(n, n1))
        ...
        if b == 1:
            return sqrt1_2 * quint(I(n1), J(n1), I(n1), -J(n1), sqrt2)
        elif b == 0:
...
cosII_neq2_mat = np.array([[1, 1], [1, -1]])
cosIII_neq2_mat = sqrt1_2 * np.array([[1, 1], [1, -1]])
